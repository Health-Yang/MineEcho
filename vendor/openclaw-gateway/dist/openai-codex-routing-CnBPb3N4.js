...
function normalizeEmbeddedAgentRuntime(raw) {
	...
	if (!value) return "pi";
	if (value === "pi" || value === "openclaw") return "pi";
...
function configuredOpenAICodexAuthProfileExists(config) {
	if (!openAIProviderUsesCodexRuntimeByDefault({
		provider: "openai",
		config
	})) return false;
	if (findNormalizedProviderValue(config?.auth?.order, "openai-codex")?.some((profileId) => typeof profileId === "string" && profileId.trim().length > 0) === true) return true;
	return Object.values(config?.auth?.profiles ?? {}).some((profile) => normalizeProviderId(profile?.provider ?? "") === OPENAI_CODEX_PROVIDER_ID);
}
...
function resolveOpenAICompactionRuntimeProvider(params) {
	if (shouldRouteOpenAIPiThroughCodexAuthProvider(params)) return OPENAI_CODEX_PROVIDER_ID;
	const runtime = normalizeEmbeddedAgentRuntime(params.agentHarnessId ?? params.harnessRuntime);
	if (!isOpenAIProvider(params.provider)) return params.provider;
	if (runtime === "codex" && (hasOpenAICodexAuthProfileOverride(params.authProfileId) || configuredOpenAIAuthOrderStartsWithCodexProfile(params.config) || configuredOpenAICodexAuthProfileExists(params.config))) return OPENAI_CODEX_PROVIDER_ID;
	return runtime === "pi" && !params.authProfileId?.trim() && configuredOpenAIAuthOrderStartsWithCodexProfile(params.config) ? OPENAI_CODEX_PROVIDER_ID : params.provider;
}
...
//#endregion
export { listOpenAIAuthProfileProvidersForAgentRuntime as a, resolveContextConfigProviderForRuntime as c, resolveSelectedOpenAIPiRuntimeProvider as d, shouldRouteOpenAIPiThroughCodexAuthProvider as f, isOpenAIProvider as i, resolveOpenAICompactionRuntimeProvider as l, resolveEmbeddedAgentRuntime as m, OPENAI_PROVIDER_ID as n, modelSelectionShouldEnsureCodexPlugin as o, normalizeEmbeddedAgentRuntime as p, isOpenAICodexProvider as r, openAIProviderUsesCodexRuntimeByDefault as s, OPENAI_CODEX_PROVIDER_ID as t, resolveOpenAIRuntimeProviderForPi as u };
