...
import { a as normalizeStateDirEnv, d as resolveGatewayPort, t as CONFIG_PATH, y as resolveStateDir } from "./paths-CQv1_CDw.js";
...
import { c as setConsoleTimestampPrefix, s as setConsoleSubsystemFilter } from "./console-D4xA24Ak.js";
...
import { a as markGatewayRestartTrace, f as startGatewayRestartTrace, o as measureGatewayRestartTrace, r as createGatewayRestartTraceHandoffEnv, t as captureGatewayRestartTraceHandoff } from "./restart-trace-Yp6H5_aR.js";
import { r as withProgress } from "./progress-Dr4m3lDw.js";
import { n as setGatewayWsLogStyle } from "./ws-logging-zK0zdycs.js";
import { n as acquireGatewayLock, t as GatewayLockError } from "./gateway-lock-DItVi1yl.js";
						...
						if (activeTasks > 0 || activeRuns > 0) {
							...
							if (restartIntent?.force) {
								...
								if (tasksDrain.drained && runsDrain.drained) gatewayLog$1.info("all active work drained");
									...
									if (!abortedAfterRunTimeout) abortEmbeddedPiRun(void 0, { mode: "all" });
								}
							}
						}
					}, () => [
...
async function resolveGatewayPasswordOption(opts) {
	...
	if (file) {
		const { readSecretFromFile } = await import("./secret-file-DoOeDJ89.js");
...
async function maybeLogPendingControlUiBuild(cfg) {
	...
	const { resolveControlUiRootSync } = await import("./control-ui-assets-BC6pmvpi.js");
...
async function maybeWriteGatewayStartupFailureBundle(err) {
	const { writeDiagnosticStabilityBundleForFailureSync } = await import("./diagnostic-stability-bundle-BIGZDpBb.js");
...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-BS2ATbmb.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-CtFqCVi-.js");
	...
	if (process.env.OPENCLAW_SERVICE_MARKER?.trim()) {
		const { cleanStaleGatewayProcessesSync } = await import("./restart-stale-pids-BeP91N0T.js");
	...
	if (opts.force) try {
		const { forceFreePortAndWait, waitForPortBindable } = await import("./ports-1PgEVzMR.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			try {
				const { formatPortDiagnostics, inspectPortUsage } = await import("./ports-BN9ylU-S.js");
				const diagnostics = await inspectPortUsage(port);
				if (diagnostics.status === "busy") for (const line of formatPortDiagnostics(diagnostics)) defaultRuntime.error(line);
			} catch {}
			const { maybeExplainGatewayServiceStop } = await import("./shared-BhNnZrwP.js");
