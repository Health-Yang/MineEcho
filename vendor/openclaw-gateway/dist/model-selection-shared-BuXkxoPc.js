...
import { i as normalizeStaticProviderModelId, r as normalizeConfiguredProviderCatalogModelId } from "./model-ref-shared-CwD1xLDF.js";
...
function listModelAliasCandidates(cfg) {
	return Object.entries(cfg.agents?.defaults?.models ?? {}).flatMap(([keyRaw, entryRaw]) => {
		...
		if (trimmedKey.endsWith("/*") && normalizeProviderId(trimmedKey.slice(0, -2))) return [];
...
function buildModelCatalogMetadata(params) {
	...
	for (const [rawKey, entryRaw] of Object.entries(configuredModels)) {
		if (rawKey.trim().endsWith("/*")) continue;
		const key = resolveAllowlistModelKey({
			cfg: params.cfg,
			raw: rawKey,
			defaultProvider: params.defaultProvider,
...
function resolveConfiguredModelRef(params) {
	...
	if (rawModel) {
		...
		const aliasCandidate = findModelAliasCandidate(params.cfg, trimmed) ?? (modelWithoutProfile && modelWithoutProfile !== trimmed ? findModelAliasCandidate(params.cfg, modelWithoutProfile) : void 0);
