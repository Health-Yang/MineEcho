...
import { t as AGENT_MODEL_CONFIG_KEYS } from "./model-refs-DEKLQX6s.js";
...
import { p as normalizeEmbeddedAgentRuntime, s as openAIProviderUsesCodexRuntimeByDefault } from "./openai-codex-routing-CnBPb3N4.js";
...
function collectCodexRuntimeModelPolicyRefs(params) {
	...
	for (const [modelRef, entry] of Object.entries(record)) {
		const trimmed = modelRef.trim();
		if (!trimmed) continue;
		if (normalizeEmbeddedAgentRuntime(normalizeOptionalLowercaseString(asOptionalRecord(asOptionalRecord(entry)?.agentRuntime)?.id)) === "codex") params.refs.push({
...
function ensureCodexRuntimePolicy(params) {
	...
	const entry = asOptionalRecord(models[params.modelRef]) ?? {};
	if (models[params.modelRef] !== entry) models[params.modelRef] = entry;
	const runtimeId = normalizeOptionalLowercaseString(asOptionalRecord(entry.agentRuntime)?.id);
