...
import { l as normalizeAgentId, r as buildAgentMainSessionKey } from "./session-key-yaI3D3Bl.js";
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-asSGjURX.js");
	if (operation === "start") {
...
async function loadOverviewForOperation(deps) {
	...
	const { loadCrestodianOverview } = await import("./overview-qkPgV64v.js");
...
async function formatOverviewForOperation(overview, deps) {
	...
	const { formatCrestodianOverview } = await import("./overview-qkPgV64v.js");
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-search") {
		...
		await (opts.deps?.runPluginsSearch ?? (async (query, pluginRuntime) => {
			const { runPluginsSearchCommand } = await import("./plugins-search-command-CLLO3gi_.js");
	...
	if (operation.kind === "setup") {
		...
		const applyDefaultModelPrimaryUpdate = setupModel.model ? (await import("./shared-ChqPPdVJ.js")).applyDefaultModelPrimaryUpdate : void 0;
	...
	if (operation.kind === "config-set") {
		...
		await (opts.deps?.runConfigSet ?? (async (setOpts) => {
			const { runConfigSet: importedRunConfigSet } = await import("./config-cli-zkHD0rxB.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-Co2vXSav.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-B8cf7x0j.js")).agentsAddCommand)({
	...
	if (operation.kind === "doctor") {
		...
		await (opts.deps?.runDoctor ?? (await import("./doctor-DRnHPJBh.js")).doctorCommand)(runtime, { nonInteractive: true });
	...
	if (operation.kind === "doctor-fix") {
		...
		const { readConfigFileSnapshot } = await import("./config/config.js");
		const before = await readConfigFileSnapshot();
		await (opts.deps?.runDoctor ?? (await import("./doctor-DRnHPJBh.js")).doctorCommand)(runtime, {
			nonInteractive: true,
			repair: true,
			yes: true
		});
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-5ENvlbgG.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui--EyLKpDz.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
	...
	if (operation.kind === "set-default-model") {
		...
		const { mutateConfigFile, readConfigFileSnapshot } = await loadConfigFileMutationHelpers();
		const before = await readConfigFileSnapshot();
		const { applyDefaultModelPrimaryUpdate } = await import("./shared-ChqPPdVJ.js");
		const result = await mutateConfigFile({
			base: "source",
			mutate: (cfg) => {
		...
		const after = await readConfigFileSnapshot();
		const { resolveAgentModelPrimaryValue } = await import("./model-input-CMOy1F_Z.js");
