...
import { s as redactSensitiveText } from "./redact-9hVaCauP.js";
...
import { d as isLinkLocalIpAddress, g as parseCanonicalIpAddress, o as isCloudMetadataIpAddress } from "./ip-Y63B6JB6.js";
import { c as shouldUseEnvHttpProxyForUrl } from "./proxy-env-Cs0G_0hd.js";
import { b as ssrfPolicyFromHttpBaseUrlAllowedOrigin, m as mergeSsrFPolicies, x as ssrfPolicyFromHttpBaseUrlFakeIpHostnameAllowlist } from "./ssrf-DcS_PaqX.js";
import { o as withTrustedEnvProxyGuardedFetchMode, r as fetchWithSsrFGuard } from "./fetch-guard-D0S6eBky.js";
import { I as resolveProviderTransportTurnStateWithPlugin } from "./provider-runtime-CGENk1Ok.js";
...
import { l as detectOpenAICompletionsCompat } from "./provider-model-compat-M50YUhbu.js";
import { t as canonicalizeBase64 } from "./base64-D03BM6T6.js";
import { n as extractToolResultId, o as isAllowedToolCallName, r as extractToolResultIds, s as normalizeAllowedToolNames, t as extractToolCallsFromAssistant } from "./tool-call-id-B9whgasj.js";
import { c as hasCopilotVisionInput, o as buildCopilotDynamicHeaders } from "./copilot-dynamic-headers-CVy4X1Kj.js";
import { c as resolveProviderRequestPolicyConfig, i as mergeModelProviderRequestOverrides, n as buildProviderRequestDispatcherPolicy, r as getModelProviderRequestTransport } from "./provider-request-config-CVigAzJO.js";
...
import { h as truncateErrorDetail } from "./provider-http-errors-B2G_gKJv.js";
import { n as ensureModelProviderLocalService } from "./provider-local-service-DglKovk7.js";
import { i as stripSystemPromptCacheBoundary } from "./system-prompt-cache-boundary-JukSqUUt.js";
import { r as resolveDebugProxySettings } from "./env-DeSKjRi7.js";
import { t as hasOpenAICompatibleConversationTurn } from "./openai-compatible-conversation-turn-DeJLw3P5.js";
...
function parseRetryAfterSeconds(headers) {
	...
	if (retryAfterMs) {
		...
		if (/^\d+(?:\.\d+)?$/.test(trimmedRetryAfterMs) && Number.isFinite(milliseconds)) return milliseconds / 1e3;
	}
...
function resolveOpenAIResponsesPayloadCapabilities(model) {
	...
	return {
		allowsOpenAIServiceTier: provider === "openai" && (api === "openai-responses" || api === "openclaw-openai-responses-transport") && endpointClass === "openai-public" || provider === "openai-codex" && (api === "openai-codex-responses" || api === "openai-responses" || api === "openclaw-openai-responses-transport") && endpointClass === "openai-codex",
...
function isInvalidEncryptedContentError(error) {
	...
	const record = error;
	if (record.code === "invalid_encrypted_content") return true;
...
function encryptedReasoningReplayMetadataMatches(metadata, context) {
	return !!metadata && metadata.provider === context.provider && metadata.api === context.api && metadata.model === context.model && metadata.baseUrlHash === context.baseUrlHash && metadata.sessionHash === context.sessionHash && metadata.authProfileHash === context.authProfileHash;
}
...
function prepareOpenAIResponsesReasoningItemForReplay(item, context, blockMetadata) {
	...
	if (!("encrypted_content" in rest)) return rest;
	if (encryptedReasoningReplayMetadataMatches(blockMetadata ?? (isOpenAIResponsesReasoningReplayMetadata(rawMetadata) ? rawMetadata : void 0), context)) return rest;
	return stripEncryptedContentFields(rest).value;
}
		...
		if (msg.role === "user") if (typeof msg.content === "string") messages.push({
			...
			for (const block of msg.content) if (block.type === "thinking") {
				if (shouldReplayReasoningItems && block.thinkingSignature && block.thinkingSignature.startsWith("{")) {
					...
					if (model.provider === "github-copilot" && !isSafeResponsesReplayItemId(replayableReasoningItem.id)) continue;
...
function clampOpenAIPromptCacheKey(key) {
	...
	return chars.length <= OPENAI_PROMPT_CACHE_KEY_MAX_LENGTH ? key : chars.slice(0, OPENAI_PROMPT_CACHE_KEY_MAX_LENGTH).join("");
}
...
function isOpenAICodexResponsesModel(model) {
	return model.provider === "openai-codex" && (model.api === "openai-codex-responses" || model.api === "openclaw-openai-responses-transport");
}
...
function buildOpenAICompletionsParams(model, context, options) {
	...
	if (compat.supportsPromptCacheKey && promptCacheKey) {
		...
		if (cacheRetention === "long") params.prompt_cache_retention = "24h";
	}
	if (options?.temperature !== void 0) params.temperature = options.temperature;
	if (options?.topP !== void 0) params.top_p = options.topP;
	...
	}
	{
		...
		if (compatDetection.capabilities.usesExplicitProxyLikeEndpoint && effectiveMaxTokens !== void 0 && effectiveContextTokens !== void 0) {
