...
import { c as isRecord$1 } from "./utils-B5r0V84N.js";
import { v as resolveSessionAgentIds } from "./agent-scope-CudANNo3.js";
import { t as classifyFailoverReason } from "./errors-DuGAmFy8.js";
import { c as resolveFailoverStatus, t as FailoverError } from "./failover-error-wBZizcCn.js";
import "./pi-embedded-helpers-Bj4nONRY.js";
import { d as maxAsk, k as resolveExecApprovalsFromFile, p as minSecurity, u as loadExecApprovals } from "./exec-approvals-oH5G9_TS.js";
import { t as extractBalancedJsonFragments } from "./balanced-json-YUc2rvlg.js";
import { n as resolveExecDefaults } from "./exec-defaults-19HKPhQW.js";
...
function buildClaudeLiveArgs(params) {
	return appendArg(upsertArgValue(upsertArgValue(upsertArgValue(stripLiveProcessArgs(params.args, params.backend, params.useResume && params.backend.systemPromptWhen !== "always"), "--input-format", "stream-json"), "--output-format", "stream-json"), "--permission-prompt-tool", "stdio"), "--replay-user-messages");
}
...
function extractClaudeEffectivePermissionMode(argv) {
...
function resolveClaudeLiveControlRequestPolicy(context, argv) {
	...
	const security = minSecurity(execDefaults.security, approvals.agent.security);
	const ask = maxAsk(execDefaults.ask, approvals.agent.ask);
	const effectivePermissionMode = (argv ? extractClaudeEffectivePermissionMode(argv) : void 0) ?? (security === "full" && ask === "off" ? CLAUDE_BYPASS_PERMISSION_MODE : void 0);
	return {
...
function shouldForceClaudeLivePermissionPrompt(policy) {
	return (policy.security !== "full" || policy.ask !== "off") && policy.effectivePermissionMode !== CLAUDE_DEFAULT_PERMISSION_MODE;
}
function shouldForceClaudeLiveBypass(policy) {
	return policy.security === "full" && policy.ask === "off" && policy.effectivePermissionMode !== CLAUDE_BYPASS_PERMISSION_MODE;
}
function writeClaudeLiveControlResponse(session, response) {
	const stdin = session.managedRun.stdin;
	if (!stdin) {
		closeLiveSession(session, "abort", /* @__PURE__ */ new Error("Claude CLI live session stdin is unavailable for control response"));
		return;
	}
	stdin.write(`${JSON.stringify(response)}\n`, (error) => {
		if (error) closeLiveSession(session, "abort", error);
	});
}
function buildClaudeLivePermissionResult(session, request) {
	...
	if (toolName === "Bash" && session.controlRequestPolicy.allowNativeBash) return {
		behavior: "allow",
		updatedInput: isRecord(request.input) ? request.input : {},
		...toolUseID ? { toolUseID } : {}
	};
	const permissionModeNote = session.controlRequestPolicy.effectivePermissionMode ? `, permission-mode=${session.controlRequestPolicy.effectivePermissionMode}` : "";
	return {
		behavior: "deny",
		message: toolName === "Bash" ? `OpenClaw denied Claude native Bash because the effective exec policy is security=${session.controlRequestPolicy.security}, ask=${session.controlRequestPolicy.ask}${permissionModeNote}; this bridge only auto-allows Bash when OpenClaw exec is full/no-ask.` : `OpenClaw denied Claude native ${toolName || "tool"}; this bridge only maps Claude native Bash permission prompts to OpenClaw exec policy. Use OpenClaw MCP tools instead.`,
		...toolUseID ? { toolUseID } : {},
		decisionClassification: "user_reject"
	};
}
function handleClaudeLiveControlRequest(session, parsed) {
	...
	if (request.subtype === "can_use_tool") {
		...
		cliBackendLog.info(`claude live control_request: subtype=can_use_tool tool=${toolName} decision=${permissionResult.behavior}`);
...
async function runClaudeLiveSessionTurn(params) {
	...
	if (shouldForceClaudeLivePermissionPrompt(launchPolicy)) {
		liveArgs = upsertArgValue(liveArgs, CLAUDE_PERMISSION_MODE_FLAG, CLAUDE_DEFAULT_PERMISSION_MODE);
