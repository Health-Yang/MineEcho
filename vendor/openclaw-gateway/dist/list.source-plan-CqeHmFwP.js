...
async function planAllModelListSources(params) {
	...
	const { loadStaticManifestCatalogRowsForList, loadSupplementalManifestCatalogRowsForList } = await import("./list.manifest-catalog-DvXyWK7X.js");
	if (!params.providerFilter) {
		const { loadProviderIndexCatalogRowsForList } = await import("./list.provider-index-catalog-CW5MZMRy.js");
		return createSourcePlan({
			kind: "registry",
	...
	const { hasProviderStaticCatalogForFilter } = await import("./list.provider-catalog-D7GoZX4j.js");
