...
import { n as DEFAULT_PROMOTION_MIN_SCORE } from "../../short-term-promotion-CDdIyVX_.js";
//#region extensions/memory-core/src/cli.ts
let memoryCliRuntimePromise = null;
async function loadMemoryCliRuntime() {
	memoryCliRuntimePromise ??= import("../../cli.runtime-COTbcSH7.js");
...
function registerMemoryCli(program) {
	...
	memory.command("promote").description("Rank short-term recalls and optionally append top entries to MEMORY.md").option("--agent <id>", "Agent id (default: default agent)").option("--limit <n>", "Max candidates", (value) => parseMemoryCliNumberOption(value, "--limit")).option("--min-score <n>", `Minimum weighted score (default: ${DEFAULT_PROMOTION_MIN_SCORE})`, (value) => parseMemoryCliNumberOption(value, "--min-score")).option("--min-recall-count <n>", `Minimum recall count (default: 3)`, (value) => parseMemoryCliNumberOption(value, "--min-recall-count")).option("--min-unique-queries <n>", `Minimum distinct query count (default: 2)`, (value) => parseMemoryCliNumberOption(value, "--min-unique-queries")).option("--apply", "Append selected candidates to MEMORY.md", false).option("--include-promoted", "Include already promoted candidates", false).option("--json", "Print JSON").action(async (opts) => {
