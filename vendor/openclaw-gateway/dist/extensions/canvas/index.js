...
import { n as isCanvasHostEnabled, t as canvasConfigSchema } from "../../config-DWV4dxt7.js";
...
function createLazyCanvasTool(params) {
	...
	const loadTool = async () => {
		toolPromise ??= import("../../tool-BeZVajW6.js").then(({ createCanvasTool }) => createCanvasTool({
	...
	register(api) {
		if (isCanvasHostEnabled(api.config)) {
			...
			const loadHttpRouteHandler = async () => {
				httpRouteHandlerPromise ??= import("../../http-route-DA0C-4N8.js").then(({ createCanvasHttpRouteHandler }) => createCanvasHttpRouteHandler({
					config: api.config,
			...
			api.registerHostedMediaResolver(async (mediaUrl) => {
				resolveCanvasHttpPathToLocalPathPromise ??= import("../../documents-3HWL4aSP.js").then(({ resolveCanvasHttpPathToLocalPath }) => resolveCanvasHttpPathToLocalPath);
		...
		api.registerNodeCliFeature(async ({ program }) => {
			const { createDefaultCanvasCliDependencies, registerNodesCanvasCommands } = await import("../../cli-Bt-8klwi.js");
