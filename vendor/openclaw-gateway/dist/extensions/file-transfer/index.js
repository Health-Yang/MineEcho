...
const loadFileTransferNodeInvokePolicy = async () => {
	const { createFileTransferNodeInvokePolicy } = await import("../../node-invoke-policy-DMHFcWEE.js");
	...
	nodeHostCommands: [
		{
			...
			handle: async (paramsJSON) => {
				const { handleFileFetch } = await import("../../file-fetch-BUVY4yIW.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirList } = await import("../../dir-list-Cq_Oz9Y7.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleDirFetch } = await import("../../dir-fetch-DaQuNDFu.js");
		...
		},
		{
			...
			dangerous: true,
			handle: async (paramsJSON) => {
				const { handleFileWrite } = await import("../../file-write-CPuKiki6.js");
