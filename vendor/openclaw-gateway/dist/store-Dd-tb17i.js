...
import { d as resolveConfigDir } from "./utils-B5r0V84N.js";
...
function persistedJobId(job) {
	return normalizeOptionalString(job.id) ?? normalizeOptionalString(job.jobId) ?? null;
}
function mergePreservedConfigJobs(jobs, preservedConfigJobs) {
	if (!preservedConfigJobs?.length) return jobs;
	const occupiedIds = /* @__PURE__ */ new Set();
	for (const job of jobs) {
		const id = persistedJobId(job);
		if (id) occupiedIds.add(id);
	}
	const seenPreservedIds = /* @__PURE__ */ new Set();
	const preserved = preservedConfigJobs.filter((entry) => {
		const id = persistedJobId(entry.job);
		if (!id) return true;
		if (occupiedIds.has(id) || seenPreservedIds.has(id)) return false;
		seenPreservedIds.add(id);
		return true;
	}).toSorted((a, b) => a.index - b.index);
	if (preserved.length === 0) return jobs;
...
function stripRuntimeOnlyCronFields(store, preservedConfigJobs) {
...
async function saveCronStore(storePath, store, opts) {
	...
	const configJson = JSON.stringify(stripRuntimeOnlyCronFields(store, opts?.preservedConfigJobs), null, 2);
...
//#endregion
export { saveCronStore as a, resolveCronStorePath as i, loadCronStoreSync as n, cronSchedulingInputsEqual as o, loadCronStoreWithConfigJobs as r, loadCronStore as t };
