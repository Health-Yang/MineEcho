import { h as shouldMigrateStateFromPath } from "./argv-86ytN5P4.js";
...
import { r as withSuppressedNotes } from "./note-C22zwxbl.js";
...
async function ensureConfigReady(params) {
	...
	if (!didRunDoctorConfigFlow && shouldMigrateStateFromPath(commandPath)) {
		...
		const runDoctorConfigPreflight = async () => (await import("./doctor-config-preflight-CVkNjwaH.js")).runDoctorConfigPreflight({
			migrateState: false,
			migrateLegacyConfig: false,
			invalidConfigNote: false
		});
		if (!params.suppressDoctorStdout) preflightSnapshot = (await runDoctorConfigPreflight()).snapshot;
		else preflightSnapshot = (await withSuppressedNotes(runDoctorConfigPreflight)).snapshot;
	}
	...
	const { formatConfigIssueLines } = await import("./issue-format-B8cDP2iB.js");
	...
	const [{ colorize, isRich, theme }, { shortenHomePath }, { formatCliCommand }, { isPluginPackagingRuntimeOutputInvalidConfigSnapshot }, { formatPluginPackagingRuntimeOutputRecoveryHint }] = await Promise.all([
		import("./theme-nS-lR20s.js"),
		import("./utils-D-IGQFBQ.js"),
		import("./command-format-AcbdTMGN.js"),
		import("./recovery-policy-Cl3IaOSf.js"),
		import("./config-recovery-hints-DuMZjDim.js")
	]);
	const rich = isRich();
