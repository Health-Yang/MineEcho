...
function applyDiscoveredContextWindows(params) {
	for (const model of params.models) {
		...
		const contextTokens = shouldUseDiscoveredAnthropicGa1MContextWindow(model) ? ANTHROPIC_CONTEXT_1M_TOKENS : discoveredContextTokens;
...
function shouldUseAnthropicGa1MContextWindow(params) {
	return isAnthropic1MModel(params.provider ? normalizeProviderId(params.provider) : "", params.model);
}
function shouldUseDiscoveredAnthropicGa1MContextWindow(model) {
	const provider = typeof model.provider === "string" ? normalizeProviderId(model.provider) : void 0;
	const modelId = model.id;
	if (provider) return isAnthropic1MModel(provider, modelId);
...
function resolveContextTokensForModel(params) {
	...
	if (explicitProvider && ref && shouldUseAnthropicGa1MContextWindow(ref)) return ANTHROPIC_CONTEXT_1M_TOKENS;
...
//#endregion
export { lookupContextTokens as a, ensureContextWindowCacheLoaded as i, applyConfiguredContextWindows as n, resolveContextTokensForModel as o, applyDiscoveredContextWindows as r, resetContextWindowCacheForTest as s, ANTHROPIC_CONTEXT_1M_TOKENS as t };
