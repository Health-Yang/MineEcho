...
import { h as shortenHomeInString } from "./utils-B5r0V84N.js";
...
import { n as collectConfiguredRuntimePluginIds, r as resolveConfiguredRuntimePluginInstallCandidate } from "./configured-runtime-plugin-installs-BjxOuG5K.js";
...
function formatBlockedRuntimePluginGuidance(params) {
	...
	const alternative = pluginId === "acpx" ? "disable ACP/acpx in acp config" : "change the runtime policy to \"pi\"";
...
function formatDisabledRuntimePluginGuidance(params) {
	...
	const alternative = params.pluginId === "acpx" ? "disable ACP/acpx in acp config" : "change the runtime policy to \"pi\"";
...
async function runPluginsEnableCommand(id) {
	...
	const { enablePluginInConfig } = await import("./enable-MV_gpADS.js");
	const { normalizePluginId } = await import("./config-state-BjcRwQD1.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-DfZA70es.js");
	const { applySlotSelectionForPlugin, logSlotWarnings } = await import("./plugins-command-helpers-DEBGvOa2.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-ChxFJdHQ.js");
...
async function runPluginsDisableCommand(id) {
	...
	const { normalizePluginId } = await import("./config-state-BjcRwQD1.js");
	const { buildPluginRegistrySnapshotReport } = await import("./status-DfZA70es.js");
	const { setPluginEnabledInConfig } = await import("./plugins-config-w3aZzj_n.js");
	const { refreshPluginRegistryAfterConfigMutation } = await import("./plugins-registry-refresh-ChxFJdHQ.js");
	const snapshot = await readConfigFileSnapshot();
	const cfg = snapshot.sourceConfig ?? snapshot.config;
	const report = buildPluginRegistrySnapshotReport({ config: cfg });
...
async function runPluginsInstallAction(raw, opts) {
	await tracePluginLifecyclePhaseAsync("install command", async () => {
		const { runPluginInstallCommand } = await import("./plugins-install-command-BynkYSja.js");
...
async function runPluginsRegistryCommand(opts) {
	const { inspectPluginRegistry, refreshPluginRegistry } = await import("./plugin-registry-BLUxsF57.js");
...
async function runPluginsDoctorCommand() {
	const { buildPluginCompatibilityNotices, buildPluginDiagnosticsReport, formatPluginCompatibilityNotice } = await import("./status-DfZA70es.js");
...
async function runPluginMarketplaceListCommand(source, opts) {
	const { listMarketplacePlugins } = await import("./marketplace-B45EQKGx.js");
	const { createPluginInstallLogger } = await import("./plugins-command-helpers-DEBGvOa2.js");
