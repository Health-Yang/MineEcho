...
async function drainCommitmentsStoreWriterQueue(storePath) {
	...
	queue.drainPromise = (async () => {
		try {
			...
			if (queue.pending.length === 0) WRITER_QUEUES.delete(storePath);
				...
				drainCommitmentsStoreWriterQueue(storePath);
			});
		}
	})();
	await queue.drainPromise;
...
async function runExclusiveCommitmentsStoreWrite(storePath, fn) {
	if (!storePath || typeof storePath !== "string") throw new Error(`runExclusiveCommitmentsStoreWrite: storePath must be a non-empty string, got ${JSON.stringify(storePath)}`);
