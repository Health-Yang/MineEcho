...
import { C as hasSessionAutoModelFallbackProvenance } from "./agent-scope-CudANNo3.js";
...
import { l as peekSystemEvents } from "./system-events-BbV9wJlR.js";
import { i as resolveCronStorePath } from "./store-Dd-tb17i.js";
import { i as summarizeRetainedLostTaskAuditFindings, r as summarizeActionableTaskAuditFindings } from "./task-registry.audit-Z_ZqEQCW.js";
import { t as areRuntimeModelRefsEquivalent } from "./model-runtime-aliases-DXHvPXsy.js";
import { r as resolveHeartbeatSummaryForAgent } from "./heartbeat-summary-DwBkJiAn.js";
import { a as createLazyRuntimeSurface } from "./lazy-runtime-D-7_JraP.js";
import { n as readSessionStoreReadOnly, t as listGatewayAgentsBasic } from "./agent-list-GOodSK5H.js";
//#region src/commands/status.summary.ts
const RECENT_SESSION_LIMIT = 10;
const channelSummaryModuleLoader = createLazyImportLoader(() => import("./channel-summary-J3NYtPoJ.js"));
const channelPluginIdsModuleLoader = createLazyImportLoader(() => import("./channel-plugin-ids---kgQpyD.js"));
const linkChannelModuleLoader = createLazyImportLoader(() => import("./status.link-channel-vGKAxPmb.js"));
const taskRegistryMaintenanceModuleLoader = createLazyImportLoader(() => import("./task-registry.maintenance-D6ziyeMD.js"));
...
function listSessionCandidates(store) {
	return Object.entries(store).filter(([key]) => key !== "global" && key !== "unknown").map(([key, entry]) => ({
		key,
		entry,
...
async function getStatusSummary(options = {}) {
	...
	const loadSessionCandidates = (storePath) => {
	...
	const buildSessionRows = (candidates, opts = {}) => candidates.map(({ key, entry, updatedAt }) => {
	...
	const byAgent = agentList.agents.map((agent) => {
		...
		const sessions = buildSessionRows(candidates.slice(0, RECENT_SESSION_LIMIT), { agentIdOverride: agent.id });
		return {
			agentId: agent.id,
			path: storePath,
	...
	const allSessions = Array.from(paths).flatMap((storePath) => loadSessionCandidates(storePath)).toSorted((a, b) => (b.updatedAt ?? 0) - (a.updatedAt ?? 0));
