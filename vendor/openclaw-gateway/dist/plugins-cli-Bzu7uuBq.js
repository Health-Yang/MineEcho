...
function registerPluginsCli(program) {
	...
	plugins.command("inspect").alias("info").description("Inspect plugin details").argument("[id]", "Plugin id").option("--all", "Inspect all plugins").option("--runtime", "Load plugin runtime for hooks/tools/diagnostics").option("--json", "Print JSON").action(async (id, opts) => {
		const { runPluginsInspectCommand } = await import("./plugins-inspect-command-4I3e-QMv.js");
	...
	plugins.command("update").description("Update installed plugins and tracked hook packs").argument("[id]", "Plugin or hook-pack id (omit with --all)").option("--all", "Update all tracked plugins and hook packs", false).option("--dry-run", "Show what would change without writing", false).option("--dangerously-force-unsafe-install", "Bypass built-in dangerous-code update blocking for plugins (plugin hooks may still block)", false).action(async (id, opts) => {
		const { runPluginUpdateCommand } = await import("./plugins-update-command-BFsv7Zo-.js");
	...
	plugins.command("build").description("Generate simple tool plugin metadata").option("--root <path>", "Plugin package root").option("--entry <path>", "Plugin entry module relative to --root").option("--check", "Fail if generated metadata is out of date", false).action(async (opts) => {
		const { runPluginsBuildCommand } = await import("./plugins-authoring-command-CDjNzJwA.js");
	...
	plugins.command("validate").description("Validate simple tool plugin metadata").option("--root <path>", "Plugin package root").option("--entry <path>", "Plugin entry module relative to --root").action(async (opts) => {
		const { runPluginsValidateCommand } = await import("./plugins-authoring-command-CDjNzJwA.js");
	...
	plugins.command("init").description("Create a simple tool plugin project").argument("<id>", "Plugin id").option("--directory <path>", "Output directory").option("--name <name>", "Display name").option("--force", "Overwrite an existing output directory", false).action(async (id, opts) => {
		const { runPluginsInitCommand } = await import("./plugins-authoring-command-CDjNzJwA.js");
