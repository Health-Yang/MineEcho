...
import { i as normalizeEnv, t as isTruthyEnvValue } from "../env-DmmbBEHd.js";
import { C as getCoreCliParentDefaultHelpCommands, O as consumeRootOptionToken, _ as getSubCliEntries, c as hasFlag, m as normalizeRootHelpTargetArgv, p as normalizeGeneratedHelpCommandArgv, v as getSubCliParentDefaultHelpCommands, x as getCoreCliCommandNames } from "../argv-86ytN5P4.js";
...
import { i as parseCliContainerArgs, n as parseCliProfileArgs, r as maybeRunCliInContainer, t as applyCliProfileEnv } from "../profile-BW4ZURCI.js";
import { t as normalizeWindowsArgv } from "../windows-argv-BmImcFTu.js";
import { n as resolveManifestCommandAliasOwnerInRegistry, r as resolveManifestToolOwnerInRegistry } from "../manifest-command-aliases-B_sF2bki.js";
...
import { a as shouldSkipPluginCommandRegistration, r as shouldRegisterPrimaryCommandOnly, t as isReservedNonPluginCommandRoot } from "../command-registration-policy-DYE4OPOG.js";
import { a as consumeGatewayFastPathRootOptionToken, n as resolveCliNetworkProxyPolicy, o as consumeGatewayRunOptionToken, t as resolveCliCommandPathPolicy } from "../command-path-policy-CgMHHnyc.js";
...
async function tryRunGatewayRunFastPath(argv, startupTrace) {
	...
	const [{ Command }, { addGatewayRunCommand }, { VERSION }, { emitCliBanner }, { resolveCliStartupPolicy }, { enableConsoleCapture }] = await startupTrace.measure("gateway-run-imports", () => Promise.all([
		import("commander"),
		import("../run-command-6wXSfwjW.js"),
		import("../version-CH9gLQAQ.js"),
		import("../banner-BDQMoNEZ.js"),
		import("../command-startup-policy-B9ifZeed.js"),
		import("../logging-WRqD4ihf.js")
	]));
...
async function closeCliMemoryManagers() {
	try {
		...
		const { closeActiveMemorySearchManagers } = await import("../memory-runtime-CRT-wKk9.js");
		await closeActiveMemorySearchManagers();
	} catch {}
}
async function disposeCliAgentHarnesses() {
	try {
		const { listAgentHarnessIds, disposeRegisteredAgentHarnesses } = await import("../registry-R0gDAxj0.js");
...
async function ensureCliEnvProxyDispatcher() {
	try {
		...
		const { ensureGlobalUndiciEnvProxyDispatcher } = await import("../undici-global-dispatcher-uQx1WAk5.js");
...
async function isPluginCliRoot(params) {
	try {
		const { resolvePluginCliRootOwnerIds } = await import("../cli-registry-loader-26PD1vVt.js");
...
async function resolveCliCommandSurfaceOwner(params) {
	const { resolveManifestCliCommandSurfaceOwner } = await import("../manifest-command-aliases.runtime-yD4HVgrs.js");
...
async function resolveUnownedCliPrimaryMessage(params) {
	const { resolveManifestCommandAliasOwner, resolveManifestToolOwner } = await import("../manifest-command-aliases.runtime-yD4HVgrs.js");
...
async function bootstrapCliProxyCaptureAndDispatcher(startupTrace, options = {}) {
	const [{ initializeDebugProxyCapture, finalizeDebugProxyCapture }, { maybeWarnAboutDebugProxyCoverage }] = await startupTrace.measure("proxy-imports", () => Promise.all([import("../runtime-BnT5kAyk.js"), import("../coverage-CzTX0QFh.js")]));
...
async function runCli(argv = process$1.argv) {
	...
	if (!isHelpOrVersionInvocation && shouldLoadCliDotEnv()) await startupTrace.measure("dotenv", async () => {
		const { loadCliDotEnv } = await import("../dotenv-CHdYNpDQ.js");
	...
	const readBestEffortCliConfig = async () => {
		if (!bestEffortConfigPromise) bestEffortConfigPromise = import("../io-7B2rS7b4.js").then(({ readBestEffortConfig }) => readBestEffortConfig());
	...
	const stopStartedProxy = async () => {
		...
		proxyHandle = null;
		if (handle) {
			const { stopProxy } = await import("../proxy-lifecycle-C9vrusRZ.js");
	...
	if (!isHelpOrVersionInvocation && shouldStartProxyForCli(normalizedArgv)) {
		...
		const { startProxy } = await import("../proxy-lifecycle-C9vrusRZ.js");
	...
	try {
		if (shouldUseRootHelpFastPath(normalizedArgv)) {
			...
			const { outputRootHelp } = await import("../root-help-SO5OSBTA.js");
		...
		if (shouldUseSetupOnboardConfigureHelpFastPath(normalizedArgv)) {
			const { tryOutputSetupOnboardConfigureHelp } = await import("../setup-onboard-configure-help-fast-path-BZ_iSHGv.js");
		...
		if (shouldRunBareRootCrestodian) {
			if (await shouldStartOnboardingForFreshInstall(normalizedArgv)) {
				...
				const { setupWizardCommand } = await import("../onboard-CvLIt0Wv.js");
			...
			const { createCliProgress } = await import("../progress-BHo4iFAp.js");
		...
		if (bootstrapProxyBeforeFastPath && await tryRunGatewayRunFastPath(normalizedArgv, startupTrace)) return;
		const { tryRouteCli } = await startupTrace.measure("route-import", () => import("../route-ORqg9u9F.js"));
		...
		try {
			const { enableConsoleCapture } = await import("../logging-WRqD4ihf.js");
			enableConsoleCapture();
			const [{ buildProgram }, { formatUncaughtError }, { formatCliFailureLines }, { runFatalErrorHooks }, { installUnhandledRejectionHandler, isBenignUncaughtExceptionError, isUncaughtExceptionHandled }, { restoreTerminalState }] = await startupTrace.measure("core-imports", () => Promise.all([
				import("../program-CO53QHpr.js"),
				import("../infra/errors.js"),
				import("../failure-output-DwAfttJi.js"),
				import("../fatal-error-hooks-BJYnE1yM.js"),
				import("../unhandled-rejections-BVC-SfjP.js"),
			...
			if (primary && shouldRegisterPrimaryCommandOnly(parseArgv)) await startupTrace.measure("register-primary", async () => {
				...
				if (ctx) {
					const { registerCoreCliByName } = await import("../command-registry-Bpz4Xrys.js");
					await registerCoreCliByName(program, ctx, primary, parseArgv);
				}
				const { registerSubCliByName } = await import("../register.subclis-CrtGhmqZ.js");
			...
			if (!shouldSkipPluginCommandRegistration({
				...
				const config = await startupTrace.measure("register-plugin-commands", async () => {
					const { registerPluginCliCommandsFromValidatedConfig } = await import("../cli--lqmoUU1.js");
