...
import { n as normalizeAgentModelRefForConfig } from "./model-input-C697EEaR.js";
...
import { a as resolvePluginSetupRegistry, i as resolvePluginSetupProvider } from "./setup-registry-Y0sCwnR-.js";
import { n as resolvePluginProviders } from "./providers.runtime-BSu6EH34.js";
import { s as loadAuthProfileStoreForRuntime } from "./store-lFmS2pvu.js";
...
import { r as promoteAuthProfileInOrder, s as upsertAuthProfileWithLock } from "./profiles-7sSr_su8.js";
import { n as clearAuthProfileCooldown } from "./usage-Dii4DVpK.js";
import { n as validateAnthropicSetupToken } from "./provider-auth-token-DFQwAe7c.js";
...
import { a as loadValidConfigOrThrow, s as resolveKnownAgentId, u as updateConfig } from "./shared-DkSSKqzU.js";
import { a as restorePriorAgentsDefaultsModelUnlessOptIn, i as resolveProviderMatch, n as applyProviderAuthConfigPatch, r as pickAuthMethod, t as applyDefaultModel } from "./provider-auth-choice-helpers-DHLn4zdG.js";
...
import { r as repairCodexRuntimePluginInstallForModelSelection } from "./codex-runtime-plugin-install-Cgw9AUVI.js";
...
function validateOpenAICodexApiKeyInput(value) {
	...
	if (looksLikeJwtToken(trimmed) || looksLikeStructuredCredential(trimmed)) return `That looks like token or OAuth material, not an OpenAI API key. Use ${formatCliCommand("openclaw models auth paste-token --provider openai-codex")} for token auth material.`;
...
async function persistProviderAuthResult(params) {
	...
	const updated = await updateConfig((cfg) => {
		...
		for (const profile of profiles) next = applyAuthProfileConfig(next, {
			profileId: profile.profileId,
			provider: profile.credential.provider,
...
async function runProviderAuthMethod(params) {
	...
	const result = await params.method.run({
		...
		openUrl: async (url) => {
			const { openUrl } = await import("./onboard-helpers-I7ctqu94.js");
			await openUrl(url);
		},
...
async function modelsAuthPasteTokenCommand(opts, runtime) {
	...
	const tokenInput = await readPastedSecret({
		...
		validate: (value) => {
			...
			if (isOpenAICodexProvider(provider) && looksLikeOpenAIApiKey(trimmed)) return `That looks like an OpenAI API key. Use ${formatCliCommand("openclaw models auth paste-api-key --provider openai-codex")} for API-key auth.`;
		}
	});
		...
		credential: {
			...
			key: await readPastedSecret({
				...
				validate: (value) => {
					const trimmed = value?.trim();
					if (!trimmed) return "Required";
					if (isOpenAICodexProvider(provider)) return validateOpenAICodexApiKeyInput(trimmed);
				}
			})
		},
		agentDir
...
//#endregion
export { modelsAuthAddCommand, modelsAuthLoginCommand, modelsAuthPasteApiKeyCommand, modelsAuthPasteTokenCommand, modelsAuthSetupTokenCommand };
