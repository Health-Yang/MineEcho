import { a as normalizeLowercaseStringOrEmpty, c as normalizeOptionalString$1, f as normalizeStringifiedOptionalString } from "./string-coerce-DKw2K5wM.js";
import { D as sanitizeCommandDescriptorDescription, E as normalizeCommandDescriptorName } from "./argv-86ytN5P4.js";
...
import { d as normalizeTrimmedStringList, f as normalizeUniqueSingleOrTrimmedStringList, h as normalizeUniqueTrimmedStringList, l as normalizeStringEntries, p as normalizeUniqueStringEntries, v as uniqueValues } from "./string-normalization-B8G0vlWE.js";
import { f as safeRealpathSync, i as isPathInside, p as safeStatSync } from "./path-BlG8lhgR.js";
import { m as shouldRejectHardlinkedPluginFiles, t as discoverOpenClawPlugins } from "./discovery-DHnXdmOi.js";
import { i as openRootFileSync } from "./root-file-jRMCpJW4.js";
import { p as resolveUserPath } from "./utils-B5r0V84N.js";
import { d as normalizeSecretInputString } from "./types.secrets-CtRQ27Ls.js";
...
import { o as resolveAgentWorkspaceDir } from "./agent-scope-config-BfxErZq2.js";
import "./boundary-file-read-CBe_wA_B.js";
import "./bundle-manifest-bztHw1KT.js";
import { _ as getPluginCompatRecord, d as collectPluginManifestCompatCodes } from "./installed-plugin-index-store-Db9FQc7C.js";
import { a as GENERATED_BUNDLED_CHANNEL_CONFIG_METADATA } from "./ids-OP3BJTaQ.js";
import { h as writeJsonSync, p as tryReadJsonSync } from "./json-files-za1pmDfK.js";
import { i as kindsEqual, n as defaultSlotIdForKey, r as hasKind } from "./slots-DT22cOei.js";
import { c as resolveEffectiveEnableState, d as resolveMemorySlotDecision, l as resolveEffectivePluginActivationState, n as createPluginActivationSource, s as normalizePluginsConfig, t as applyTestPluginDefaults } from "./config-state-CzzsKZJi.js";
import { t as isPluginEnabledByDefaultForPlatform } from "./default-enablement-CT7_t1ee.js";
import { r as loadInstalledPluginIndexInstallRecordsSync } from "./installed-plugin-index-record-reader-CQGWoRBr.js";
import { t as PluginLruCache } from "./plugin-cache-primitives-DcK62VYf.js";
import { t as loadPluginManifestRegistry } from "./manifest-registry-DYz4Sf21.js";
import { C as fingerprintPluginDiscoveryContext, T as resolvePluginDiscoveryContext } from "./plugin-registry-BALJ9TgI.js";
import { n as getCachedPluginModuleLoader, s as toSafeImportPath, t as createPluginModuleLoaderCache } from "./plugin-module-loader-cache-OiY7Cg2j.js";
import { _ as shouldPreferNativeModuleLoad, a as listPluginSdkAliasCandidates, c as resolveExtensionApiAlias, f as resolvePluginRuntimeModulePath, g as resolvePluginSdkScopedAliasMap, h as resolvePluginSdkAliasFile, m as resolvePluginSdkAliasCandidateOrder, n as buildPluginLoaderJitiOptions, o as listPluginSdkExportedSubpaths, p as resolvePluginRuntimeModulePathWithDiagnostics, t as buildPluginLoaderAliasMap } from "./sdk-alias-B5GyZdZF.js";
...
import { t as validateJsonSchemaValue } from "./schema-validator-ocH5WSQX.js";
import { n as listChatChannels } from "./chat-meta-DIhiKDzc.js";
...
import { a as serializePluginIdScope, i as normalizePluginIdScope, n as hasExplicitPluginIdScope, t as createPluginIdScopeSet } from "./plugin-scope-Cg_NWH2k.js";
import { m as normalizeChannelMeta } from "./bundled-CTRQpZ0m.js";
...
import { t as buildPluginApi } from "./api-builder-BUAAgdVs.js";
import { a as listRegisteredAgentHarnesses, c as restoreRegisteredAgentHarnesses, o as registerAgentHarness, r as getRegisteredAgentHarness, t as clearAgentHarnesses } from "./registry-CA-J0IzZ.js";
import { G as resolveMemoryDreamingConfig, K as resolveMemoryDreamingPluginConfig, _ as DEFAULT_MEMORY_DREAMING_PLUGIN_ID } from "./dreaming-DT202ka-.js";
...
import { t as createPluginStateKeyedStore } from "./plugin-state-store-BVbTs5d2.js";
import { i as emitAgentEvent } from "./agent-events-BuYtWSh4.js";
import { t as isPluginJsonValue } from "./host-hook-json-BHsxXV1L.js";
import { a as normalizeAgentToolResultMiddlewareRuntimeIds, n as normalizePluginToolContractNames, o as normalizeAgentToolResultMiddlewareRuntimes, r as normalizePluginToolNames, t as findUndeclaredPluginToolNames } from "./tool-contracts-B95kI03B.js";
import { n as resolveWorkspaceRoot, r as resolvePathFromInput } from "./workspace-dir-D3ilNvO8.js";
import { n as resolveAgentMainSessionKey } from "./main-session-896o98eb.js";
...
import { l as normalizeSessionEntrySlotKey, t as loadSessionStore } from "./store-load-Di_AfnVx.js";
...
import "./host-hook-cleanup-tMxkYloX.js";
...
function getRegisteredEmbeddingProvider(id) {
	return getEmbeddingProviders().get(id) ?? CORE_EMBEDDING_PROVIDERS.find((entry) => entry.adapter.id === id);
}
function listRegisteredEmbeddingProviders() {
	const merged = new Map(CORE_EMBEDDING_PROVIDERS.map((entry) => [entry.adapter.id, entry]));
	for (const entry of getEmbeddingProviders().values()) merged.set(entry.adapter.id, entry);
...
function registerNativeAlias(params) {
	...
	for (const parentRoot of params.parentRoots) {
		if (entries.some((entry) => entry.parentRoot === parentRoot && entry.target === params.target)) continue;
		entries.push({
			parentRoot,
...
function emitPluginAgentEvent(params) {
	const runId = normalizeOptionalString$1(params.event.runId);
	const sessionKey = normalizeOptionalString$1(params.event.sessionKey);
	const stream = normalizeOptionalString$1(params.event.stream);
...
function resolveGeneratedBundledChannelMeta(id) {
	...
	const label = normalizeOptionalString$1(channel?.label);
	...
	return {
		...
		blurb: normalizeOptionalString$1(channel.description) ?? ""
	};
}
function collectMissingChannelMetaFields(meta) {
	const missing = [];
	if (!normalizeOptionalString$1(meta?.label)) missing.push("label");
	if (!normalizeOptionalString$1(meta?.selectionLabel)) missing.push("selectionLabel");
	if (!normalizeOptionalString$1(meta?.docsPath)) missing.push("docsPath");
	if (typeof meta?.blurb !== "string") missing.push("blurb");
	return missing;
}
function normalizeRegisteredChannelPlugin(params) {
	const id = normalizeOptionalString$1(params.plugin?.id) ?? normalizeStringifiedOptionalString(params.plugin?.id) ?? "";
	if (!id) {
	...
	const rawMetaId = normalizeOptionalString$1(rawMeta?.id);
...
async function loadSendMessage() {
	sendMessagePromise ??= import("./message-BAP2Xw9P.js").then((module) => module.sendMessage);
	return sendMessagePromise;
}
let getChannelPluginPromise;
async function loadGetChannelPlugin() {
	getChannelPluginPromise ??= import("./plugins-BogMrHXo.js").then((module) => module.getChannelPlugin);
...
function resolveAttachmentDelivery(params) {
	...
	if (channel === "slack") {
		...
		const threadTs = normalizeOptionalString$1(hint?.threadTs);
		return {
...
async function validateAttachmentFiles(files, maxBytes, options) {
	...
	for (const file of files) {
		...
		const filePath = normalizeOptionalString$1(file.path);
...
function normalizeOptionalThreadId(value) {
	...
	return normalizeOptionalString$1(value);
}
...
async function sendPluginSessionAttachment(params) {
	...
	const rawText = normalizeOptionalString$1(params.text) ?? "";
...
function resolveSchedule(params) {
	const cron = normalizeOptionalString$1(params.cron);
	if (cron) {
		const tz = normalizeOptionalString$1(params.tz);
		return {
			kind: "cron",
			expr: cron,
			...tz ? { tz } : {}
...
function resolvePluginSessionTurnTag(value) {
	const tag = normalizeOptionalString$1(value);
...
async function schedulePluginSessionTurn(params) {
	if (params.origin !== "bundled") return;
	const sessionKey = normalizeOptionalString$1(params.schedule.sessionKey);
	const message = normalizeOptionalString$1(params.schedule.message);
	...
	const scheduleName = normalizeOptionalString$1(params.schedule.name);
...
async function unschedulePluginSessionTurnsByTag(params) {
	...
	const sessionKey = normalizeOptionalString$1(params.request.sessionKey);
...
function findFreshestStoreMatch(store, ...candidates) {
	...
	for (const candidate of candidates) {
		const trimmed = normalizeOptionalString$1(candidate) ?? "";
		if (!trimmed) continue;
...
function loadPluginHostHookSessionEntry(params) {
	const key = normalizeOptionalString$1(params.sessionKey) ?? "";
...
async function patchPluginSessionExtension(params) {
	...
	const rawSlotKey = normalizeOptionalString$1(registration.extension.sessionEntrySlotKey);
...
function uniqueModels(provider) {
	return normalizeUniqueSingleOrTrimmedStringList([provider.defaultModel, ...provider.models ?? []]);
}
...
function createModelCatalogRegistrationHandlers(params) {
	const registerModelCatalogProvider = (record, provider) => {
		const providerId = normalizeOptionalString$1(provider.provider) ?? "";
		if (!providerId) {
			params.pushDiagnostic({
				level: "error",
				pluginId: record.id,
...
function normalizeProviderOAuthProfileIdRepairs(values) {
	...
	const normalized = values.map((value) => {
		const legacyProfileId = normalizeOptionalString$1(value?.legacyProfileId);
		const promptLabel = normalizeOptionalString$1(value?.promptLabel);
...
function buildNormalizedModelAllowlist(modelAllowlist) {
	...
	const message = normalizeOptionalString$1(modelAllowlist.message);
...
function buildNormalizedWizardSetup(params) {
	const choiceId = normalizeOptionalString$1(params.setup.choiceId);
	const choiceLabel = normalizeOptionalString$1(params.setup.choiceLabel);
	const choiceHint = normalizeOptionalString$1(params.setup.choiceHint);
	const groupId = normalizeOptionalString$1(params.setup.groupId);
	const groupLabel = normalizeOptionalString$1(params.setup.groupLabel);
	const groupHint = normalizeOptionalString$1(params.setup.groupHint);
...
function buildNormalizedModelPicker(modelPicker, methodId) {
	const label = normalizeOptionalString$1(modelPicker.label);
	const hint = normalizeOptionalString$1(modelPicker.hint);
	return {
		...label ? { label } : {},
		...hint ? { hint } : {},
		...methodId ? { methodId } : {}
...
function normalizeProviderWizardSetup(params) {
	...
	const methodId = resolveWizardMethodId({
		...
		source: params.source,
		auth: params.auth,
		methodId: normalizeOptionalString$1(params.setup.methodId),
		metadataKind: "setup",
		pushDiagnostic: params.pushDiagnostic
	});
...
function normalizeProviderAuthMethods(params) {
	...
	for (const method of params.auth) {
		const methodId = normalizeOptionalString$1(method.id);
		if (!methodId) {
			pushPluginValidationDiagnostic({
				level: "error",
				pluginId: params.pluginId,
		...
		normalized.push({
			...method,
			id: methodId,
			label: normalizeOptionalString$1(method.label) ?? methodId,
			...normalizeOptionalString$1(method.hint) ? { hint: normalizeOptionalString$1(method.hint) } : {},
			...wizard ? { wizard } : {}
		});
	}
	return normalized;
...
function normalizeProviderWizard(params) {
	...
	const normalizeModelPicker = () => {
		...
		return buildNormalizedModelPicker(modelPicker, resolveWizardMethodId({
			...
			source: params.source,
			auth: params.auth,
			methodId: normalizeOptionalString$1(modelPicker.methodId),
...
function normalizeRegisteredProvider(params) {
	const id = normalizeOptionalString$1(params.provider.id);
	if (!id) {
		pushPluginValidationDiagnostic({
			level: "error",
			pluginId: params.pluginId,
	...
	const docsPath = normalizeOptionalString$1(params.provider.docsPath);
	...
	return {
		...restProvider,
		id,
		label: normalizeOptionalString$1(params.provider.label) ?? id,
		...docsPath ? { docsPath } : {},
		...aliases ? { aliases } : {},
...
function createPluginRegistry(registryParams) {
	...
	const describeHttpRouteOwner = (entry) => {
		return `${normalizeOptionalString$1(entry.pluginId) || "unknown-plugin"} (${normalizeOptionalString$1(entry.source) || "unknown-source"})`;
	};
	...
	const registerAgentHarness$1 = (record, harness) => {
		const id = normalizeOptionalString$1(harness?.id) ?? "";
		if (!id) {
			pushDiagnostic({
				level: "error",
				pluginId: record.id,
	...
	const registerNodeHostCommand = (record, nodeCommand) => {
		...
		registry.nodeHostCommands.push({
			...
			command: {
				...nodeCommand,
				command,
				cap: normalizeOptionalString$1(nodeCommand.cap)
			},
			source: record.source,
			rootDir: record.rootDir
		});
			...
			handlers: {
				...registrationCapabilities.capabilityHandlers ? {
					...
					registerContextEngine: (id, factory) => {
						const normalizedId = normalizeOptionalString$1(id) ?? "";
						if (!normalizedId) {
							pushDiagnostic({
								level: "error",
								pluginId: record.id,
					...
					registerCompactionProvider: (provider) => {
						const id = normalizeOptionalString$1(provider?.id);
						if (!id) {
							pushDiagnostic({
								level: "error",
								pluginId: record.id,
...
function loadOpenClawPlugins(options = {}) {
	...
	try {
		...
		const resolveCreatePluginRuntime = () => {
			...
			const runtimeModuleResolution = resolvePluginRuntimeModulePathWithDiagnostics({ pluginSdkResolution: options.pluginSdkResolution });
...
//#endregion
export { getRegisteredEmbeddingProvider as A, getRegisteredMemoryEmbeddingProvider as C, restoreRegisteredMemoryEmbeddingProviders as D, registerMemoryEmbeddingProvider as E, resolvePluginActivationSourceConfig as F, registerEmbeddingProvider as M, restoreRegisteredEmbeddingProviders as N, PluginLoadReentryError as O, getCompactionProvider as P, getMemoryEmbeddingProvider as S, listRegisteredMemoryEmbeddingProviders as T, getPluginSessionExtensionStateSync as _, getRuntimePluginRegistryForLoadOptions as a, listCodexAppServerExtensionFactories as b, loadOpenClawPlugins as c, resolveRuntimePluginRegistry as d, testing as f, drainPluginNextTurnInjectionContext as g, synthesizeMediaGenerationCatalogEntries as h, clearPluginRegistryLoadCache as i, listRegisteredEmbeddingProviders as j, clearEmbeddingProviders as k, resolveCompatibleRuntimePluginRegistry as l, listMediaGenerationProviderModels as m, clearActivatedPluginRuntimeState as n, isPluginRegistryLoadInFlight as o, createPluginRegistry as p, clearPluginLoaderCache as r, loadOpenClawPluginCliRegistry as s, PluginLoadFailureError as t, resolvePluginRegistryLoadCacheKey as u, patchPluginSessionExtension as v, listMemoryEmbeddingProviders as w, clearMemoryEmbeddingProviders as x, projectPluginSessionExtensionsSync as y };
