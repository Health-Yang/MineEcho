import { n as detectErrorKind } from "./errors-BsfWgA0I.js";
import { a as isSubagentSessionKey, n as isAcpSessionKey } from "./session-key-utils-AmKO9Roj.js";
...
import { c as loadSessionEntry, s as loadGatewaySessionRow } from "./session-utils-CmwnszsS.js";
import { d as stripHeartbeatToken } from "./heartbeat-CFoWrjC8.js";
import { a as resolveToolSearchCodeDisplayTarget } from "./tool-display-common-pmkk3Oly.js";
...
import { a as createToolEventRecipientRegistry, i as createSessionMessageSubscriberRegistry, n as createChatRunState, r as createSessionEventSubscriberRegistry, t as createChatRunRegistry } from "./server-chat-state-5p6_r5uT.js";
...
function createAgentEventHandler({ broadcast, broadcastToConnIds, nodeSendToSession, agentRunSeq, chatRunState, resolveSessionKeyForRun, clearAgentRunContext, toolEventRecipients, sessionEventSubscribers, sessionMessageSubscribers, loadGatewaySessionRowForSnapshot = loadGatewaySessionRow, lifecycleErrorRetryGraceMs = AGENT_LIFECYCLE_ERROR_RETRY_GRACE_MS, isChatSendRunActive = () => false }) {
	...
	const finalizeLifecycleEvent = (evt, opts) => {
		...
		if (sessionKey && (isControlUiVisible || sessionMessageSubscribers.get(sessionKey).size > 0)) if (!isAborted) {
			...
			if (chatLink) {
				...
				if (!(opts?.skipChatErrorFinal && lifecyclePhase === "error")) emitChatFinal(finished.sessionKey, finished.clientRunId, evt.runId, evt.seq, lifecyclePhase === "error" ? "error" : "done", evt.data?.error, evtStopReason, evtErrorKind, { controlUiVisible: isControlUiVisible });
			} else if (!(opts?.skipChatErrorFinal && lifecyclePhase === "error")) emitChatFinal(sessionKey, eventRunId, evt.runId, evt.seq, lifecyclePhase === "error" ? "error" : "done", evt.data?.error, evtStopReason, evtErrorKind, { controlUiVisible: isControlUiVisible });
		} else {
	...
	const emitChatDelta = (sessionKey, clientRunId, sourceRunId, seq, text, delta, opts) => {
	...
	const flushBufferedChatDeltaIfNeeded = (sessionKey, clientRunId, sourceRunId, seq, opts) => {
	...
	const sendNodeAgentPayload = (sessionKey, payload) => {
		if (sessionKey) nodeSendToSession(sessionKey, "agent", payload);
	};
	...
	return (evt) => {
		...
		const hasSessionMessageSubscribers = sessionKey ? sessionMessageSubscribers.get(sessionKey).size > 0 : false;
		...
		if (isToolEvent) {
			if ((typeof evt.data?.phase === "string" ? evt.data.phase : "") === "start" && (isControlUiVisible || hasSessionMessageSubscribers) && sessionKey && !isAborted && !suppressHeartbeatToolEvents) {
				flushBufferedChatDeltaIfNeeded(sessionKey, clientRunId, evt.runId, evt.seq, { controlUiVisible: isControlUiVisible });
				flushBufferedAgentDeltaIfNeeded(clientRunId);
			}
			...
			if ((isItemEvent && typeof evt.data?.phase === "string" ? evt.data.phase : "") === "start" && (isControlUiVisible || hasSessionMessageSubscribers) && !isAborted) {
				if (sessionKey) flushBufferedChatDeltaIfNeeded(sessionKey, clientRunId, evt.runId, evt.seq, { controlUiVisible: isControlUiVisible });
				flushBufferedAgentDeltaIfNeeded(clientRunId);
			}
		...
		if ((isControlUiVisible || hasSessionMessageSubscribers) && sessionKey) {
			...
			if (!isAborted && evt.stream === "assistant" && typeof evt.data?.text === "string" && !shouldSuppressAssistantEventForLiveChat(evt.data)) emitChatDelta(sessionKey, clientRunId, evt.runId, evt.seq, evt.data.text, evt.data.delta, { controlUiVisible: isControlUiVisible });
		}
