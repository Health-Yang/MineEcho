...
import { r as createConfigIO } from "./io-BlARNTf3.js";
import { t as PLUGIN_INSTALLS_CONFIG_PATH } from "./installed-plugin-index-records-BV-e8pNZ.js";
...
import { l as formatPluginCompatibilityNotice, r as buildPluginCompatibilitySnapshotNotices } from "./status-D3q_e-nF.js";
import { n as commitConfigWriteWithPendingPluginInstalls } from "./plugins-install-record-commit-DTf3q6zq.js";
import { t as resolveSetupSecretInputString } from "./setup.secret-input-BJNiGPSt.js";
import { r as runSetupMigrationImport, t as detectSetupMigrationSources } from "./setup.migration-import-B0-2VGKG.js";
...
function isPluginInstallsUnsetPath(path) {
	return path.length === PLUGIN_INSTALLS_CONFIG_PATH.length && path.every((part, index) => part === PLUGIN_INSTALLS_CONFIG_PATH[index]);
}
function loadAuthChoiceModule() {
	authChoiceModulePromise ??= import("./auth-choice-C4SWiXSC.js");
	return authChoiceModulePromise;
}
function loadConfigLoggingModule() {
	configLoggingModulePromise ??= import("./logging-NsVlIej4.js");
	return configLoggingModulePromise;
}
function loadModelPickerModule() {
	modelPickerModulePromise ??= import("./model-picker-Du-cduU8.js");
...
async function writeWizardConfigFile(config, opts = {}) {
	...
	return (await commitConfigWriteWithPendingPluginInstalls({
		...
		commit: async (nextConfig, writeOptions) => {
			const allowPluginInstallMigrationSizeDrop = writeOptions?.unsetPaths?.some(isPluginInstallsUnsetPath) === true;
...
async function resolveAuthChoiceModelSelectionPolicy(params) {
	...
	const [{ resolveManifestProviderAuthChoice }, { resolvePluginSetupProvider }] = await Promise.all([import("./provider-auth-choices-BceJzsAA.js"), import("./setup-registry-BNQ0JE3y.js")]);
...
async function runSetupWizard(opts, runtime, prompter) {
	...
	const onboardHelpers = await import("./onboard-helpers-I7ctqu94.js");
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-E78GNa0x.js");
		const { applySkipBootstrapConfig } = await import("./onboard-config-BgSe6l5B.js");
		...
		nextConfig = await writeWizardConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	...
	const { applyLocalSetupWorkspaceConfig, applySkipBootstrapConfig } = await import("./onboard-config-BgSe6l5B.js");
	...
	if (authChoiceFromPrompt) {
		...
		({promptAuthChoiceGrouped} = await import("./auth-choice-prompt-Cfv7ZeEw.js"));
	...
	while (true) {
		...
		if (authChoice === "custom-api-key") {
			const { promptCustomApiConfig } = await import("./onboard-custom-CjviJ0KW.js");
	...
	const { configureGatewayForSetup } = await import("./setup.gateway-config-Bxx-IM0x.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note(t("wizard.setup.skipChannels"), t("wizard.setup.channelsTitle"));
		...
		const { listChannelPlugins } = await import("./plugins-BogMrHXo.js");
		const { setupChannels } = await import("./onboard-channels-Bt5AIH-9.js");
	...
	if (opts.skipSearch) await prompter.note(t("wizard.setup.skipSearch"), t("wizard.setup.searchTitle"));
		...
		const { setupSearch } = await import("./onboard-search-Zo8GkxoL.js");
	...
	if (opts.skipSkills) await prompter.note(t("wizard.setup.skipSkills"), t("wizard.setup.skillsTitle"));
		...
		const { setupSkills } = await import("./onboard-skills-CPVf9Zy6.js");
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	if (flow !== "quickstart") {
		const { setupOfficialPluginInstalls } = await import("./setup.official-plugins-y2trtxdP.js");
		...
		const { setupPluginConfig } = await import("./setup.plugin-config-C4Ur1FlK.js");
	...
	if (!opts.skipHooks) {
		const { setupInternalHooks } = await import("./onboard-hooks-crMbgZfg.js");
	...
	nextConfig = await writeWizardConfigFile(nextConfig, { allowConfigSizeDrop: configResetPerformed });
	const { finalizeSetupWizard } = await import("./setup.finalize-BBksSXzb.js");
